...
function Details() {
  ...
  const getGenres = async () => {
    const genreUrl = "https://api.themoviedb.org/3/genre/tv/list?api_key=58b1cda8f42c9d9f9f2d707739ee0c6c&language=en-US";
  ...
  const getCast = async () => {
    const castUrl = `https://api.themoviedb.org/3/tv/${state.show.id}/credits?api_key=58b1cda8f42c9d9f9f2d707739ee0c6c&language=en-US`;
  ...
  const getVideo = async () => {
    const videoUrl = `https://api.themoviedb.org/3/tv/${state.show.id}/videos?api_key=58b1cda8f42c9d9f9f2d707739ee0c6c&language=en-US`;
  ...
  return (
    <div className='detail-page'>
      <div className='detail-bg-img'
        style={
          {
            backgroundImage: `url(https://image.tmdb.org/t/p/w1280/${state.show.backdrop_path})`,
      ...
      <div className='detail-poster'>
        <img src={"https://image.tmdb.org/t/p/w500" + state.show.poster_path} className='detail-poster-img' />
        <div className='movie-content-info'>
          <div className='title'>
            <span>{state.show.name}</span>
          </div>
          <div className='genres'>
            {
              state.show.genre_ids.map((category) => {
                {
                  let check = "";
          ...
          <p className='individual-overview'>{state.show.overview}</p>
          <h4>Casts</h4>
          <div className="cast">
            {cast.map((actor) => {
              return (
